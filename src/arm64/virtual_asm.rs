use crate::arm64::asm;
use crate::arm64::asm::{Asm, CompBin, Exp, FCompBin, Fundef, IdOrImm, Prog};
use crate::closure;
use crate::id;
use crate::syntax;
use closure::Closure;
 ...
 * S: State
 */
fn classify<S, T, F1: Fn(&mut S, T, String) -> T, F2: Fn(&mut S, T, String, Type) -> T>(
    state: &mut S,
    xts: Vec<(String, Type)>,
    init: T,
    addf: F1,
...
fn separate(xts: &[(String, Type)]) -> (Box<[String]>, Box<[String]>) {
    let (intargs, floatargs) = classify(
        &mut (),
        xts.to_vec(),
        (Vec::new(), Vec::new()),
        |_, (intargs, mut floatargs), x| {
            floatargs.push(x);
            (intargs, floatargs)
        },
        |_, (mut intargs, floatargs), x, _| {
            intargs.push(x);
            (intargs, floatargs)
        },
    );
...
fn expand<
    T,
    F1: Fn(&mut IdGen, String, i32, T) -> T,
    F2: Fn(&mut IdGen, String, Type, i32, T) -> T,
>(
    id_gen: &mut IdGen,
    xts: &[(String, Type)],
    init: (i32, T),
...
fn g(
    data: &mut Vec<(id::L, OrderedFloat<f64>)>,
    ...
    match e {
        ...
        Closure::IfComp(op, x, y, e1, e2) => match env.get(&x) {
        ...
        Closure::MakeCls(
            ...
            let ys = ys
                .into_vec()
                .into_iter()
                .map(|y| (y.clone(), env.get(&y).unwrap().clone()))
                .collect::<Vec<_>>();
            let (offset, store_fv) = expand(
                id_gen,
                &ys,
                (4, e2p),
                |id_gen, y, offset, store_fv| {
                    asm::seq(
                        id_gen,
                        Exp::StDF(y, x.clone(), IdOrImm::C(offset), 1),
                        store_fv,
                    )
                },
                |id_gen, y, _, offset, store_fv| {
                    asm::seq(
                        id_gen,
                        Exp::St(y, x.clone(), IdOrImm::C(offset), 1),
                        store_fv,
                    )
                },
            );
            ...
            Asm::Let(
                ...
                Box::new(Asm::Let(
                    ...
                    Box::new(Let(
                        ...
                        Box::new(asm::seq(
                            id_gen,
                            Exp::St(z, x.clone(), IdOrImm::C(0), 1),
                            store_fv,
                        )),
                    )),
                )),
        ...
        Closure::Tuple(xs) => {
            use crate::arm64::asm::{align, REG_HP};
            let y = id_gen.gen_id("t");
            let xs_with_type = xs
                .iter()
                .map(|x| {
                    let tmp = env.get(x).unwrap();
                    (x.clone(), tmp.clone())
                })
                .collect::<Vec<_>>();
            let (offset, store) = expand(
                ...
                &xs_with_type,
                (0, Ans(Mov(y.clone()))),
                |id_gen, x, offset, store| {
                    asm::seq(id_gen, StDF(x, y.clone(), IdOrImm::C(offset), 1), store)
                },
                |id_gen, x, _, offset, store| {
                    asm::seq(id_gen, St(x, y.clone(), IdOrImm::C(offset), 1), store)
                },
            );
            Let(
                y,
        ...
        Closure::Get(x, y) => match env.get(&x) {
            Some(Type::Array(ty)) => match ty as &Type {
                Type::Unit => Asm::Ans(Exp::Nop),
                Type::Float => Asm::Ans(Exp::LdDF(x, IdOrImm::V(y), 8)),
                _ => Asm::Ans(Exp::Ld(x, IdOrImm::V(y), 4)),
            },
            _ => panic!("invalid type for an array (Get)"),
        },
        Closure::Put(x, y, z) => match env.get(&x) {
            Some(Type::Array(ty)) => match ty as &Type {
                Type::Unit => Asm::Ans(Exp::Nop),
                Type::Float => Asm::Ans(Exp::StDF(z, x, IdOrImm::V(y), 8)),
                _ => Asm::Ans(Exp::St(z, x, IdOrImm::V(y), 4)),
            },
            _ => panic!("invalid type for an array (Put)"),
        },
        Closure::ExtArray(id::L(x)) => Ans(SetL(id::L(format!("min_caml_{}", x)))),
    }
}
...
fn h(
    ...
    let (_offset, load) = expand(
        ...
        (4, inner_e),
        |_, z, offset, load| {
            crate::arm64::asm::fletd(z, Exp::LdDF(x.clone(), IdOrImm::C(offset), 1), load)
        },
        |_, z, t, offset, load| {
            Asm::Let(
                z,
                t,
                Exp::Ld(x.clone(), IdOrImm::C(offset), 1),
                Box::new(load),
            )
        },
    );
    match t {
        Type::Fun(_, t2) => Fundef {
            name: id::L(x),
            args: intargs,
            fargs: floatargs,
            body: load,
...
mod tests {
    ...
    fn test_comparison() {
        ...
        let expected = Asm::Ans(Exp::IfComp(
            CompBin::LE,
            x(),
            IdOrImm::V(y()),
            int_wrapper(3),
    ...
    fn test_make_closure() {
        ...
        let expected;
        {
            ...
            let e4 = Asm::Let(
                "u0".to_string(),
                Type::Unit,
                Exp::St(x(), func(), IdOrImm::C(4), 1),
                Box::new(e5),
            );
            let e3 = Asm::Let(
                "u1".to_string(),
                Type::Unit,
                Exp::St(y(), func(), IdOrImm::C(8), 1),
                Box::new(e4),
            );
            let e2 = Asm::Let(
                "u3".to_string(),
                Type::Unit,
                Exp::St("l2".to_string(), func(), IdOrImm::C(0), 1),
                Box::new(e3),
            );
            let e1 = Asm::Let(
                "l2".to_string(),
                Type::Int,
                Exp::SetL(id::L("f".to_string())),
                Box::new(e2),
            );
            let e0 = Asm::Let(
                "min_caml_hp".to_string(),
                Type::Int,
                Exp::IntOp(IntBin::Add, "min_caml_hp".to_string(), IdOrImm::C(16)),
                Box::new(e1),
            );
            expected = Asm::Let(
                func(),
                func_type,
                Exp::Mov("min_caml_hp".to_string()),
                Box::new(e0),
            );
        }
        assert_eq!(expected, g(&mut data, &env, e, &mut id_gen));
