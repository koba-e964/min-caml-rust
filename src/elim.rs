...
fn has_effect(e: &KNormal) -> bool {
    use self::KNormal::*;
    match e {
        Let(_, e1, e2) | IfComp(_, _, _, e1, e2) => has_effect(e1) || has_effect(e2),
...
mod tests {
    ...
    fn test_unnecessary_let() {
        ...
        let e = Let(
            (x(), Type::Int),
            Box::new(Int(1)),
            Box::new(Let((y(), Type::Int), Box::new(Var(x())), Box::new(Int(1)))),
        );
        let e2 = Int(1);
        assert_eq!(f(e), e2);
    }
    ...
    fn test_unnecessary_lettuple() {
        ...
        // 6
        let e = LetTuple(
            Box::new([("x".to_string(), Type::Int), ("y".to_string(), Type::Int)]),
            "z".to_string(),
            Box::new(Int(6)),
        );
        let e2 = Int(6);
