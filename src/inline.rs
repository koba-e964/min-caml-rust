...
pub fn size(e: &KNormal) -> usize {
    use self::KNormal::*;
    match e {
        IfComp(_, _, _, e1, e2) | Let(_, e1, e2) | LetRec(KFundef { body: e1, .. }, e2) => {
            1 + size(e1) + size(e2)
        }
...
fn g(env: &TypeEnv, e: KNormal, id_gen: &mut IdGen, inline_threshold: usize) -> KNormal {
    use self::KNormal::*;
    macro_rules! invoke {
        ($e: expr) => {
            Box::new(g(env, *$e, id_gen, inline_threshold))
        };
    }
    match e {
        IfComp(op, x, y, e1, e2) => IfComp(op, x, y, invoke!(e1), invoke!(e2)),
        Let((x, t), e1, e2) => Let((x, t), invoke!(e1), invoke!(e2)),
        LetRec(
            KFundef {
                name: (x, t),
                args: yts,
            ...
            let is_rec = is_recursive(
                &(KFundef {
                    name: (x.clone(), t.clone()),
                    args: yts.clone(),
                    body: e1.clone(),
                }),
            ); // Apparently these clone()s should be unnecessary.
